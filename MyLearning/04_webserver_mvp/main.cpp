...
int main(int argc, char *argv[]) {
  ...
  int epollfd = epoll_create(5); // epoll_create1
  ...
  while (true) {
    ...
    for (int i = 0; i < number; i++) {
      ...
      // 🚨 情况 C：客人发来了 HTTP 请求 (EPOLLIN).
      else if (events[i].events & EPOLLIN) {
        // 叫服务员去读数据
        if (users[sockfd].read_once()) {
          // 读完之后，把这个服务员扔进线程池，让打工人去解析报文
          pool->append(users + sockfd); // pool->append( &users[sockfd] );
        } else {
          // 读失败了，赶客
          users[sockfd].close_conn();
        }
